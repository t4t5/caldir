...
pub async fn handle(cmd: ListEvents) -> Result<Vec<Event>> {
    ...
    let path = format!(
        "/me/calendars/{}/calendarView?startDateTime={}&endDateTime={}&$top=100&$select=id,iCalUId,subject,body,start,end,location,isAllDay,isCancelled,recurrence,attendees,organizer,reminderMinutesBeforeStart,showAs,lastModifiedDateTime,onlineMeeting,originalStart",
        config.outlook_calendar_id, cmd.from, cmd.to
    );
