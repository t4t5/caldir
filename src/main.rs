...
enum Commands {
    ...
    /// Sync events from cloud to local directory
    Sync,
...
async fn cmd_auth(provider: &str) -> Result<()> {
    match provider {
        "gcal" => {
            ...
            let email = providers::gcal::fetch_user_email(&tokens).await?;
            println!("\nAuthenticated as: {}", email);

            // Fetch and display calendars
            let calendars = providers::gcal::fetch_calendars(&tokens).await?;
            println!("\nAvailable calendars:");
            ...
            println!("You can now run `caldir-sync sync` to sync your calendar.");
...
async fn cmd_sync() -> Result<()> {
    ...
    let calendars = providers::gcal::fetch_calendars(gcal_tokens).await?;

    println!("Connected to Google Calendar. Found {} calendars.", calendars.len());
    println!("\nSync not yet implemented - coming soon!");
