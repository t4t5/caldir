...
**Caldir** — The top-level context handle (`caldir_core::caldir::Caldir`). Holds the global config (time format, default calendar, default reminders, plus the source path the config was loaded from) and the discovered provider binaries. **This is the project's context object** — every CLI command receives a `&Caldir` and threads it (or `caldir.config()`) down to lower layers. Two entry points: production uses `Caldir::load()`, which resolves the platform config path and discovers providers on `PATH`; tests use `Caldir::new(config, providers)` (the `mock_caldir()` helper produces a tempdir-rooted config). `caldir.save_config()` delegates to `caldir.config().save()`, which writes back to the path the config carries — so a `caldir.config().clone()` mutated locally and saved still goes to the right file. `CaldirEnvironment` is the internal loader that backs `Caldir::load()` — tests don't reach for it. Factory methods: `caldir.calendar(slug)`, `caldir.calendars()`, `caldir.new_calendar(slug, config)`, `caldir.unique_slug_for(name)`.
