...
**Caldir** — The top-level context handle (`caldir_core::caldir::Caldir`). Holds a resolved `CaldirEnvironment` with runtime paths and discovered provider binaries. **This is the project's context object** — every CLI command receives a `&Caldir` and threads it (or `caldir.environment()`) down to lower layers. Production uses `Caldir::load()`, which loads `CaldirEnvironment` from the TOML config, resolves provider data/search paths, and discovers providers. Tests use `Caldir::new(environment)` or the `mock_caldir()` helper, which returns a tempdir-owning test wrapper. `CaldirConfig` remains the pure TOML schema; `CaldirEnvironment` composes it with runtime-only values like `config_path`, resolved paths, and the provider snapshot. `caldir.save_config()` delegates to `caldir.environment().save_config()`, which writes the inner config back to its configured path. Factory methods: `caldir.calendar(slug)`, `caldir.calendars()`, `caldir.new_calendar(slug, config)`, `caldir.unique_slug_for(name)`.
