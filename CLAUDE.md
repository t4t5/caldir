...
**Caldir** — The top-level context handle (`caldir_core::caldir::Caldir`). Holds the global config (time format, default calendar, default reminders), the data path (`~/caldir`), the config store it saves back to, and the discovered provider binaries. **This is the project's context object** — every CLI command receives a `&Caldir` and threads it (or `caldir.config()`) down to lower layers. Production code uses `Caldir::load()` for the default process environment. Tests and alternate embedders use `Caldir::new(CaldirOptions { ... })` with an explicit config, config store (`File` or `Memory`), and provider list. `CaldirEnvironment` is the loader boundary that resolves platform config paths, provider search paths, and provider data directories before producing `CaldirOptions`. Factory methods: `caldir.calendar(slug)`, `caldir.calendars()`, `caldir.new_calendar(slug, config)`, `caldir.unique_slug_for(name)`.
