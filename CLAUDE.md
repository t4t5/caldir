...
**Caldir** — The top-level context handle (`caldir_core::caldir::Caldir`). Holds the global config (time format, default calendar, default reminders), the data path (`~/caldir`), and the config-file path it was loaded from. **This is the project's context object** — every CLI command receives a `&Caldir` and threads it (or `caldir.config()`) down to lower layers. There is no zero-arg `Caldir::load()` that resolves paths from global state; callers must pass an explicit config path (`Caldir::load(CaldirConfig::config_path()?)?` at CLI entry, `Caldir::with_data_path(tmp)` in tests). This makes every read of the platform config dir auditable, and prevents tests from clobbering the user's real `~/.config/caldir/config.toml`. Factory methods: `caldir.calendar(slug)`, `caldir.calendars()`, `caldir.new_calendar(slug, config)`, `caldir.unique_slug_for(name)`.
