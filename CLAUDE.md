...
**Caldir** — The top-level context handle (`caldir_core::caldir::Caldir`). Holds the resolved runtime state: config path, parsed `CaldirConfig`, expanded calendar/provider data paths, and the discovered provider snapshot. **This is the project's context object** — every CLI command receives a `&Caldir` and threads it down to lower layers. Production uses `Caldir::load()`, which reads the TOML config, resolves paths, and discovers providers from `PATH`. Tests and GUI hosts use `Caldir::builder()` to inject a config path, in-memory `CaldirConfig`, provider search dirs, or an explicit provider snapshot. `CaldirConfig` remains the pure TOML schema. `caldir.save_config()` writes the current config back to `caldir.config_path()`. Factory methods: `caldir.calendar(slug)`, `caldir.calendars()`, `caldir.new_calendar(slug, config)`, `caldir.unique_slug_for(name)`.

**Calendar** — Represents a single calendar directory (`caldir_core::calendar::Calendar`). Holds its absolute directory path explicitly (no global lookups inside methods). Construct via `caldir.calendar(slug)` in production, `Calendar::load(slug, caldir_dir)` directly in tests (with a tempdir). Reads the local config from `.caldir/config.toml` if present, otherwise falls back to `CalendarConfig::default()`. Provides methods for event CRUD operations and sync state management. The `remote()` method returns `Option<Remote>` — calendars without a configured remote are local-only.

**CalendarDiff** — Bidirectional diff for a single calendar (`caldir_core::diff::CalendarDiff`). Created via `CalendarDiff::from_calendar(caldir, &cal, &range).await`. Contains `to_push` and `to_pull` vectors of `EventDiff`. Call `apply_push().await` or `apply_pull()` to sync changes.
