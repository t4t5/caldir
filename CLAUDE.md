...
**Caldir** — The top-level context handle (`caldir_core::caldir::Caldir`). Holds resolved `CaldirSettings` and the discovered provider binaries. **This is the project's context object** — every CLI command receives a `&Caldir` and threads it (or `caldir.settings()`) down to lower layers. Production uses `Caldir::load()`, which loads `CaldirSettings`, resolves provider data/search paths, and discovers providers. Tests use `Caldir::new(settings, providers)` or the `mock_caldir()` helper, which returns a tempdir-owning test wrapper. `CaldirConfig` remains the pure TOML schema; `CaldirSettings` composes it with runtime-only values like `config_path` and provider search dirs, and resolves paths on demand. `caldir.save_config()` delegates to `caldir.settings().save_config()`, which writes the inner config back to its configured path. Factory methods: `caldir.calendar(slug)`, `caldir.calendars()`, `caldir.new_calendar(slug, config)`, `caldir.unique_slug_for(name)`.
