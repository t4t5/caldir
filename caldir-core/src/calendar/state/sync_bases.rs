...
#[derive(Debug)]
pub struct SyncBases(HashMap<EventInstanceId, Option<Box<Event>>>);
...
impl SyncBases {
    ...
    pub fn insert_known_event_id(&mut self, id: EventInstanceId) {
        self.0.insert(id, None);
    }

    pub fn insert_event_base(&mut self, id: EventInstanceId, event: Event) {
        self.0.insert(id, Some(Box::new(event)));
    }

    pub fn get(&self, id: &EventInstanceId) -> Option<&Option<Box<Event>>> {
        self.0.get(id)
    }

    pub fn iter(&self) -> impl Iterator<Item = (&EventInstanceId, &Option<Box<Event>>)> {
        self.0.iter()
    }

    pub fn load_from_state_dir(state_dir: &Path) -> Result<Self, CalendarStateError> {
